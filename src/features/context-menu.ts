  ...
  return {
    ...
    setup(ctx) {
      ...
      function show(x: number, y: number): void {
        ...
        // Paste needs the clipboard feature; omit it on iOS touch (the native
        // long-press callout owns paste there).
        if (clipboard && !(clipboard.isIOS && lastPointerType === "touch")) {
          addButton("Paste", () => {
            clipboard.paste();
          });
        }
        if (menu.childElementCount === 0) {
          return; // nothing to offer (iOS without a selection)
        }
      ...
      // Track pointer type so the iOS paste gating and long-press know the input.
      ...
      // Long-press for touch devices without a native callout. iOS has its own
      // callout (Select/Copy/Paste) and would race ours, so skip the timer there.
      const isIOS = clip()?.isIOS ?? false;
